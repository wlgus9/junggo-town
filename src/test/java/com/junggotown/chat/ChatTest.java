...
public class ChatTest {
    ...
    @Nested
    @DisplayName("채팅 전송할 때")
    class Send {
        @Test
        @DisplayName("성공이면 응답 메세지가 ResponseMessage.CHAT_SEND_SUCCESS와 같다")
        void chatSendSuccess() throws Exception {
            saveProduct();
            ...
            JsonNode response1_1 = TestUtil.performPostRequestAndGetResponse(
                    mockMvc, SEND_URL, consumerUserId, consumerUserToken, chatDto1_1, HttpStatus.OK
            );
            JsonNode response1_2 = TestUtil.performPostRequestAndGetResponse(
                    mockMvc, SEND_URL, registUserId1, registUserToken1, chatDto1_2, HttpStatus.OK
            );
            JsonNode response1_3 = TestUtil.performPostRequestAndGetResponse(
                    mockMvc, SEND_URL, consumerUserId, consumerUserToken, chatDto1_3, HttpStatus.OK
            );

            JsonNode response2_1 = TestUtil.performPostRequestAndGetResponse(
                    mockMvc, SEND_URL, consumerUserId, consumerUserToken, chatDto2_1, HttpStatus.OK
            );
            JsonNode response2_2 = TestUtil.performPostRequestAndGetResponse(
                    mockMvc, SEND_URL, registUserId2, registUserToken2, chatDto2_2, HttpStatus.OK
            );
            JsonNode response2_3 = TestUtil.performPostRequestAndGetResponse(
                    mockMvc, SEND_URL, consumerUserId, consumerUserToken, chatDto2_3, HttpStatus.OK
            );
        ...
        @Test
        @DisplayName("구매자가 먼저 채팅을 보내면 응답 메세지가 ResponseMessage.PRODUCT_BUYER_IS_NOT_EXISTS와 같다")
        void chatSendFailFirstChatBySeller() throws Exception {
            saveProduct();

            ChatDto chatDto = ChatDto.getCreateDto(1L, registUserId1, consumerUserId, "안녕하세요");
            JsonNode response = TestUtil.performPostRequestAndGetResponse(
                    mockMvc, SEND_URL, registUserId1, registUserToken1, chatDto, HttpStatus.BAD_REQUEST
            );
            assertThat(response.get("message").asText()).isEqualTo(ResponseMessage.PRODUCT_BUYER_IS_NOT_EXISTS.getMessage());
        }
    }

    @Nested
    @DisplayName("채팅 조회할 때")
    class Search {
        @Test
        @DisplayName("성공이면 응답 메세지가 ResponseMessage.CHAT_SEARCH_SUCCESS와 같다")
        void searchChatSuccess() throws Exception {
            saveChat();

            JsonNode response = TestUtil.performGetRequestAndGetResponse(
                    mockMvc, SEARCH_URL+"1", consumerUserId, consumerUserToken, HttpStatus.OK
            );
            assertThat(response.get("message").asText()).isEqualTo(ResponseMessage.CHAT_SEARCH_SUCCESS.getMessage());
        }

        @Test
        @DisplayName("없으면 응답 메세지가 ResponseMessage.CHAT_IS_EMPTY와 같다")
        void searchChatFailIsEmpty() throws Exception {
            saveChat();

            JsonNode response = TestUtil.performGetRequestAndGetResponse(
                    mockMvc, SEARCH_URL+"4", consumerUserId, consumerUserToken, HttpStatus.OK
            );
    ...
    @Nested
    @DisplayName("채팅 전체 조회할 때")
    class SearchAll {
        @Test
        @DisplayName("성공이면 응답 메세지가 ResponseMessage.CHAT_SEARCH_SUCCESS와 같다")
        void searchAllChatSuccess() throws Exception{
            saveChat();

            JsonNode response = TestUtil.performGetRequestAndGetResponse(
                    mockMvc, SEARCH_ALL_URL, consumerUserId, consumerUserToken, HttpStatus.OK
            );
            assertThat(response.get("message").asText()).isEqualTo(ResponseMessage.CHAT_SEARCH_SUCCESS.getMessage());
        }
        ...
        @Test
        @DisplayName("없으면 응답 메세지가 ResponseMessage.CHAT_IS_EMPTY와 같다")
        void searchAllChatFailIsEmpty() throws Exception{
            JsonNode response = TestUtil.performGetRequestAndGetResponse(
                    mockMvc, SEARCH_ALL_URL, consumerUserId, consumerUserToken, HttpStatus.OK
            );
            assertThat(response.get("message").asText()).isEqualTo(ResponseMessage.CHAT_IS_EMPTY.getMessage());
    ...
    void saveProduct() throws Exception {
        ...
        TestUtil.performPostRequestAndGetResponse(
                mockMvc, CREATE_URL, registUserId1, registUserToken1, productDto1, HttpStatus.OK
        );
        TestUtil.performPostRequestAndGetResponse(
                mockMvc, CREATE_URL, registUserId1, registUserToken1, productDto2, HttpStatus.OK
        );
        TestUtil.performPostRequestAndGetResponse(
                mockMvc, CREATE_URL, registUserId2, registUserToken2, productDto3, HttpStatus.OK
        );
    }
